...
@Riverpod(keepAlive: true)
class SettingSportsValue extends _$SettingSportsValue {
  @override
  String build() {
    return ref.read(sharedPrefManagerProvider).getSportName() ?? kSportName;
  }

  set setSports(String sport) {
    ref.read(sharedPrefManagerProvider).setSportName(sport);
    state = sport;
  }

  String get sport => state;
}

@Riverpod(keepAlive: true)
class SportList extends _$SportList {
  @override
  List<ClubLocationSports> build() {
    return [];
  }

  set sport(List<ClubLocationSports>? sport) {
    state = sport ?? state;
  }
